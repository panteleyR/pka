...
//M-221(отказался от нее)
//y^2 = x^3+117050x^2+x
//modulo p = 2^221 - 3

//secp256k1, исп}ользуется в битке
$a=gmp_init(0);
$b=gmp_init(7);
$mod = gmp_init('115792089237316195423570985008687907853269984665640564039457584007908834671663');
$pointG = new Point(gmp_init('55066263022277343669578718895168534326250603453777594175500187360389116729240'), gmp_init('32670510020758816978083085130507043184471273380659243275938904335757337482424'), $mod);
$q = gmp_init('115792089237316195423570985008687907852837564279074904382605163141518161494337');
...
$m = TextHexBinConverter::convertTextToHex($inputText);
$zero = TextHexBinConverter::getHexStartZero($m);
$m = gmp_init($m, 16);
//print('Сообщение: '.gmp_strval($m, 16)."<br><br>");

$c = gmp_random_range(gmp_init(1), gmp_sub($q,1));
$Db = $calc->multiplyByN($pointG, $c);
//printf('Закрытый ключ c=%s <br> Открытый ключ Db = (%s, %s)<br><br>', $c, $Db->getX(), $Db->getY());
$k = gmp_random_range(gmp_init(1), gmp_sub($q,1));
$R = $calc->multiplyByN($pointG, $k);
$P = $calc->multiplyByN($Db, $k);
$e = gmp_mod(gmp_mul($m, $P->getX()), $mod);
printf('Параметр k=%s <br> R = kG = (%s, %s)<br> P=kDB = (%s, %s)<br>', $k, $R->getX(), $R->getY(), $P->getX(), $P->getY());
printf('e=mx (mod p) = %s <br><br>', $e);

//(R,e)

$Q = $calc->multiplyByN($R, $c);
printf('Q = cR = (%s, %s) <br>', $Q->getX(), $Q->getY());
$euc = new Euclidean($Q->getX(), $mod);
$m = $euc->euclideanReverseNumber($e);
...
class PointCalculate
{
    ...
    public function poryadokPointG(Point $g)
    {
        ...
        while (!$point instanceof NullablePoint) {
            if (gmp_strval(gmp_mod($k,gmp_init('10000000')), 10) === '0') {
                print_r(gmp_strval($k));
            }
            $point = $this->sum($g,$g);
            $k = gmp_add($k, 1);
        }
